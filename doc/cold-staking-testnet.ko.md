...
테스트넷 높이 `12000`부터 콜드 스테이킹 보상 자동 복리가 활성화됩니다. 당첨된
계약 UTXO는 소비된 뒤 별도 보상 UTXO를 추가하지 않고 `원금 + 보상` 금액의 계약
UTXO 하나로 다시 생성됩니다. 따라서 스테이킹이 성공해도 위임 UTXO 개수는
증가하지 않습니다. 활성화 전에 만들어진 출력은 그대로 유효하며 자동으로
합쳐지지는 않습니다.
...
- 자동 복리 활성화 이후에는 별도 금액이 있는 coinbase 보상 출력이 생기지 않고,
  계약 출력 금액만 정확한 합의 보상만큼 증가하며 UTXO 수가 유지됩니다.
...
- 콜드 스테이킹 보상 자동 복리: 높이 `12000`부터 활성화
